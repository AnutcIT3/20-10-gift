...
test('admin can sign in, open the seating map, and assign an empty seat', async ({ page }) => {
  ...
  await page.route('**/*', async (route) => {
    ...
    if (url.pathname === '/api/admin/data-revision') {
      await route.fulfill({ json: { success: true, data: { revision } } })
      return
    }
    ...
    if (url.pathname === '/api/students/22/seat' && request.method() === 'PATCH') {
      ...
      Object.assign(students[1], { seat_row: body.seat_row, seat_col: body.seat_col })
      revision += 1
      await route.fulfill({
        json: {
          success: true,
  ...
  students[1].full_name = 'Hương cập nhật'
  revision += 1
  await expect(page.getByRole('button', { name: 'Hương cập nhật (12A1)' })).toBeVisible({ timeout: 7000 })
})
